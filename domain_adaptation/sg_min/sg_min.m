function [fn,Yn,AY,obj,num_iter,flag,time] = sg_min(Y0,varargin)
...
% SGParameters.motion = 0 for approximate, 1 for exact
	...
	if (~gradtolarg)
		SGParameters.gradtol = 1;%10;%1e-7, 0.9;
	end
	if (~ftolarg)
		SGParameters.ftol = 1e-5; %0.9;%1e-10;
	end
	if (~mdarg)
		SGParameters.Mode = 0;
	end
% Make a partition using a possible given one given one
	...
	elseif (SGParameters.Mode == 2)
		[fn,Yn,AY,obj,num_iter,flag,time] = sg_prcg(Y0);
